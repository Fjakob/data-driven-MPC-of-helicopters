...
#define NCONTROLINPUTS  429 //710 (uncondensed) /**< Number of opt variables. */
#define MAXITER         100	    				/**< Maximum number of iterations. */
#define HESSIANTYPE     HST_SEMIDEF			    /**< Hessian type, see documentation of SQProblem class constructor. */
#define MAXCPU          2                     /**< Maximum cpu time*/
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
	...
	SQProblem* problem;
	real_t *H, *g, *A, *lb, *ub, *lbA, *ubA, *cputime;
	...
	else
	{
		...
		if ( ( status != SUCCESSFUL_RETURN ) && ( status != RET_MAX_NWSR_REACHED ) )
		{
			...
			status = problem->init( H,g,A,lb,ub,lbA,ubA, nWSR_retry, cputime);
            nWSR += nWSR_retry;
		}
